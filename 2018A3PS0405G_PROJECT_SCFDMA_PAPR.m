% Code to simuulate PAPR for SCFDMA
clc
clear
...
for n = 1:numRuns
    ...
    % Convert data to frequency domain for SCFDMA
    X=fft(inputSymbols);
    ...
    Y2(1:Q_tilda:Q_tilda*numSymbols) = X;
    % IDFT
    y = ifft(Y);
    y1 = ifft(Y1);
    y2 = ifft(Y2);
...
%  xlabel('PAPR')
%  ylabel('CCDF')
%  title('LFDMA - SCFDMA PAPR')
%  subplot(3,1,2)
%  semilogy(X1(1:end-1),1-cumsum(N1)/max(cumsum(N1)),'-g') % For IFDMA
%  xlabel('PAPR')
%  ylabel('CCDF')
%  title('IFDMA - SCFDMA PAPR')
%  subplot(3,1,3)
%  semilogy(X2(1:end-1),1-cumsum(N2)/max(cumsum(N2)),'-b') % For DFDMA
%  xlabel('PAPR')
%  ylabel('CCDF')
%  title('DFDMA - SCFDMA PAPR')