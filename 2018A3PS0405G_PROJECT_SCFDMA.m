% Code to simulate SER vs SNR [dB] for SCFDMA
clc
clear
SP.subband = 0;
...
for n=1:length(SP.SNR)
    ...
    for k=1:SP.numRun
        ...
        % Subcarrier mapping. 
        Y(1:SP.inputBlockSize) = inputSymbols_freq; %LFDMA
        Y1(1:Q:numSymbols) = inputSymbols_freq; %IFDMA
        Y2(1:Q_tilda:Q_tilda*SP.inputBlockSize) = inputSymbols_freq; %DFDMA
        ...
        %IFFT for SCFDMA
        Y2_lfdma = ifft(Y1_lfdma);
        Y2_ifdma = ifft(Y1_ifdma);
        Y2_dfdma = ifft(Y1_dfdma);
        % Perform demodulation
        EstData_lfdma = qamdemod(Y2_lfdma, M, 'UnitAveragePower', true);
        EstData_ifdma = qamdemod(Y2_ifdma, M, 'UnitAveragePower', true);
        EstData_dfdma = qamdemod(Y2_dfdma, M, 'UnitAveragePower', true);
        % Error counting
